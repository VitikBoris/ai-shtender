...
| `users/` (опционально) | Состояние пользователя (режим меню) | `users/{chat_id}.json` |

Примечание: `users/` нужен, если есть режимы `/menu` (например «обработка фото» vs «рамка») и нужно помнить выбор между сообщениями.
...
## 4.4. Формат `users/{chat_id}.json` (при /menu)
...
## 4.5. Lifecycle Policy (автоочистка)
