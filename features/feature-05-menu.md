...
**Статус:** ✅ Реализовано (две кнопки)  
**Коммит:** —

## Цель

Реализовать команду `/menu`, которая показывает пользователю меню с **двумя** вариантами и кнопкой «Назад»:

1. **Детализация** — обработка фото через Replicate (реставрация/улучшение), затем отправка результата и при наличии шаблона — генерация штендера (лицо + PDF).
2. **Создание штендера** — только детекция лица по фото и генерация PDF-штендера (без Replicate).
3. **Кнопка «Назад»** — закрыть меню (убрать клавиатуру).
...
- Предыдущий пункт: [4.5 Генерация штендера PDF](feature-04.5-shtender-pdf.md)